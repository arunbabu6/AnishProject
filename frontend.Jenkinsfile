pipeline {
    ...
    stages {
        ...
        stage('Stash Client') {
            ...
            steps {
                dir('frontend/studio-ghibli') {
                    stash includes: '**', name: 'frontendaneesh-src'
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    dir('frontend/studio-ghibli') {
                        ...
                        stash excludes: 'node_modules/**', includes: '**', name: 'build-artifacts2'
                    }
                }
            }
        }
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                withSonarQubeEnv('Sonarqube') { // 'Sonarcube-cred' from |should match the SonarQube configuration in Jenkins
                    sh """
                      sonar-scanner \
                      -Dsonar.projectKey=Project-Aneesh \
                      -Dsonar.sources=. \
        ...
        //        snykSecurity failOnError: false, failOnIssues: false, organisation: 'arunbabu6', projectName: 'For-Green2', snykInstallation: 'Snyk', snykTokenId: 'snyk-token', targetFile: 'package.json'
                snykSecurity failOnError: false, failOnIssues: false, organisation: 'arunbabu6', projectName: 'For-Aneesh', snykInstallation: 'Snyk', snykTokenId: 'snyk-token'
                }
                    ...
                    dir('artifacts2') {
                        ...
                        withCredentials([usernamePassword(credentialsId: 'dockerhub1', usernameVariable: 'DOCKER_USERNAME', passwordVariable: 'DOCKER_PASSWORD')]) {
                            sh """
                                ...
                                ssh ab@host.docker.internal 'docker push ${env.DOCKER_IMAGE}-frontend-aneesh:${env.ENVIRONMENT.toLowerCase()}-${env.BUILD_NUMBER}'
                            """
                        }
            ...
            steps {
                script {
                    ...
                    sshagent(['jenkinaccess']) {
                        ...
                        echo \"Scanning ${env.DOCKER_IMAGE}-frontend-aneesh:${env.ENVIRONMENT.toLowerCase()}-${env.BUILD_NUMBER} with Trivy...\" && \
                        trivy image --format json --output \"/opt/docker-aneesh/Trivy/trivy-report--${env.BUILD_NUMBER}.json\" ${env.DOCKER_IMAGE}-frontend:${env.ENVIRONMENT.toLowerCase()}-${env.BUILD_NUMBER}'"
                        ...
                        case 'Demo':
                            sshagent(['jenkinaccess']) {
                                sh """
                                    ...
                                    docker run -d --name projectname-frontend -p 8091:80 ${env.DOCKER_IMAGE}-frontend-aneesh:${env.ENVIRONMENT.toLowerCase()}-${env.BUILD_NUMBER}
                                    '
                            """
                            }
                            break
